...
class Invoice(Base):
    ...
    # External resource (personnel factuurvoorstel); null for customer sales invoices.
...
class MonthlyCost(Base):
    """Operational cost line: one-off for a month, or recurring across months."""
    ...
    start_month: Mapped[str] = mapped_column(String(7), index=True)
    # YYYY-MM — last inclusive month for recurring; null = ongoing
